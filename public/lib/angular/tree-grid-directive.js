    ...
    .run([
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          ...
          "   </thead>\n" +
          "   <tbody>\n" +
          "     <tr ng-init=\"index = $index\" ng-repeat=\"row in tree_rows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
          "       ng-class=\"'level-' + {{ row.level }} + (row.branch.selected ? ' active':'')\" class=\"tree-grid-row\">\n" +
          "       <td  ng-class=\"{'bg-red' :selections.indexOf(index) > -1}\"><div style=\"width:300px;\"><a ng-click=\"user_clicks_branch(row.branch)\"><i ng-class=\"row.tree_icon\"\n" +
          "              ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
          "              class=\"indented tree-icon\"></i>\n" +
          "           </a><span class=\"indented tree-label\" ng-click=\"on_user_click(row.branch)\">\n" +
          "             {{row.branch[expandingProperty.field] || row.branch[expandingProperty]}}</span></div>\n" +
          "       </td>\n" +
          "       <td ng-class=\"{'bg-red' : selections.indexOf(index) > -1}\" ng-repeat=\"col in colDefinitions\">\n" +
      ...
      function ($timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            ...
            attrs.iconLeaf = attrs.iconLeaf ? attrs.iconLeaf : '';
